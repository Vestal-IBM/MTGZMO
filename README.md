...
- Yaskawa A1000 VFD control over MEMOBUS/Modbus RTU (RS-485): run, stop, set frequency
- VFD fault and alarm monitoring — on-screen overlay notification on any new trip
...
> Configure A1000 parameters: **H5-01** = slave address (default 1), **H5-02** = baud rate (default 9600), **b1-01** = 2 (Modbus frequency reference), **b1-02** = 2 (Modbus run command).
...
| VFD max frequency | `60.00 Hz` | Upper limit for the frequency setpoint |
...
|---|---|---|
| H5-01 | 1 (default) | Slave address — must match web UI setting |
...
When a new fault is detected, the fault name is shown on the display overlay for 6 s. When a fault clears, "VFD Fault Cleared" is shown for 3 s. Common faults decoded on-device:
...
- **VFD — Yaskawa A1000** card — shows live VFD status (Running / Stopped / No comms) and current frequency setpoint. Controls: frequency input, **Run**, **Stop**, **Reset** (fault reset). Status auto-refreshes every 2 s.
...
- **VFD Settings** card — set the Modbus slave address (1–31) and maximum frequency limit (Hz). Persists to EEPROM.
- **WiFi** card — switch between AP and Station mode. In Station mode enter your router SSID and password; if connection fails within 10 s the device falls back to AP mode automatically. The Station credentials are preserved even after a fallback.
...
| `/vfd-run` | POST | — | Send run command to VFD |
| `/vfd-stop` | POST | — | Send stop command to VFD |
| `/vfd-freq` | POST | `hz` | Set VFD frequency reference (0–400 Hz) |
| `/vfd-reset` | POST | — | Reset active VFD fault |
| `/vfd-settings` | POST | `slave`, `maxhz` | Set Modbus slave address and max frequency |
| `/vfd-status` | GET | — | Returns JSON: `{ok, status, fault, running, freq}` |
...
> The magic byte is checked on every boot. If it does not match `0xAF`, all settings revert to firmware defaults and EEPROM is re-initialised on next save.
